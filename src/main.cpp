...
static
void matrix_print_text(text_params_t_st *params);

static
void run_countdown_tests(Cmd *thisCmd, char *command, bool printHelp);

static
void run_running_text_test(Cmd *thisCmd, char *command, bool printHelp);

static
void run_fill_screen_test(Cmd *thisCmd, char *command, bool printHelp);

static
void print_screen_init_message(void);

static
void print_test_completed(void);
...
void print_help(Cmd *thisCmd, char *command, bool printHelp) {
  ...
  Serial.print("\trun_running_text_test <delay_ms>: \t\t\t Runs a scrolling text test\r\n");
...
/**
 * @brief Run running text test on the LED matrix panel
 * @param Cmd pointer to command object
 * @param command Command string
 * @param printHelp Flag indicating whether to print help
 */
...
void run_running_text_test(Cmd *thisCmd, char *command, bool printHelp) {
  text_params_t_st params = {0};
  const char *str = "This is a long text that should scroll across the display from right to left for testing purposes.";
...
void setup() {
  ...
  cmd->AddCmd(PSTR("help"), print_help);
	cmd->AddCmd(PSTR("run_running_text_test"), run_running_text_test);
